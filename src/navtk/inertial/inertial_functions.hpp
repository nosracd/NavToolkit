...
Vector3 calc_force_and_acceleration_offset(const S1& r_e,
	...
	return {-v_ned0(1) * (2 * omega + lambda_dot) * sin_l + v_ned0(2) * l_dot + g(0),
	        // Equation 3.77, pp 52, Titterton text
	        v_ned0(0) * (2 * omega + lambda_dot) * sin_l +
	            v_ned0(2) * (2 * omega + lambda_dot) * cos_l + g(1),
	        // Equation 3.78, pp 52, Titterton text
	        -v_ned0(1) * (2 * omega + lambda_dot) * cos_l - v_ned0(0) * l_dot + g(2)};
}
 ...
 * @tparam std::enable_if_t<> Constrains B1, B2, B3, B4, B6, and B7 to be 1-dimensional
 * _and the B5 and B8 to be 2-dimensional.
 *
 * @param r_e east_distances, shape (N).  Can accept initializer lists.
 * @param r_n north_distances, shape (N).  Can accept initializer lists.
 * @param alt0 The inertial's current ellipsoidal altitudes, m, shape (N).  Can accept initializer
 * lists.
 * @param cos_l Cosines of inertial latitudes, shape (N).  Can accept initializer lists.
 * @param g Estimated local gravities along North, East and Down axes, m/s^2, shape (N, 3).  Can
 * accept initializer lists.
 * @param sec_l Secants of inertial latitudes, shape (N).  Can accept initializer lists.
 * @param sin_l Sines of inertial latitudes, shape (N).  Can accept initializer lists.
...
template <typename B1 = Vector,
          ...
          typename B7 = Vector,
          typename B8 = Matrix,
          IfAllConditions<TENSORS_ARE_DIM<1, B1, B2, B3, B4, B6, B7>, TENSORS_ARE_DIM<2, B5, B8>>* =
              nullptr>
Matrix calc_force_and_acceleration_offset(const B1& r_e,
	...
	for (size_t i = 0; i < N; i++) {
		l_dot      = v_ned0(i, 0) / (r_n(i) + alt0(i));
		lambda_dot = v_ned0(i, 1) * sec_l(i) / (r_e(i) + alt0(i));

		offset(i, 0) =
		    -v_ned0(i, 1) * (2 * omega + lambda_dot) * sin_l(i) + v_ned0(i, 2) * l_dot + g(i, 0);
		// Equation 3.77, pp 52, Titterton text
		offset(i, 1) = v_ned0(i, 0) * (2 * omega + lambda_dot) * sin_l(i) +
		               v_ned0(i, 2) * (2 * omega + lambda_dot) * cos_l(i) + g(i, 1);
		// Equation 3.78, pp 52, Titterton text
		offset(i, 2) =
		    -v_ned0(i, 1) * (2 * omega + lambda_dot) * cos_l(i) - v_ned0(i, 0) * l_dot + g(i, 2);
	}
